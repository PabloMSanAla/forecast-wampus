...
creates .d files for each plane, and then runs the executable_lc for each plane-snapshot pair in parallel.
...
Example usage:
  ./lc.sh -n 4 -i 36 -lc ../lc/planes_list.txt
...
# Function to run executable_lc
run_lc() {
    local plane=$1
    local snapshot=$2
    echo "Processing plane number: $plane with snapshot: $snapshot"
    
    ./executable_lc ${snapshot} ${plane}
...
do
    ...
    then
        plane_num="${planes[$i]}"
        echo "${snaps[$i]} ${plane_num}"
    fi        
done | parallel --bar --jobs $num_processors --colsep ' ' run_lc {1} {2}
