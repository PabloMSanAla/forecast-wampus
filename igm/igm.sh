...
help() {
    cat << EOF
          ...
          ~~~~~  FORECAST IGM module batch processing script  ~~~~~

This script runs the IGM module for multiple planes in parallel using GNU parallel.
...
and then runs executable_igm for each plane-snapshot pair in parallel.

Usage: ./igm.sh [options]

Options:
  -n: Number of processors to use (default: 1)
  -i: Initial plane number (default: 0)
  -lc: Path to the planes_list.txt file (default: from igm.ini or ../lc/planes_list.txt)
  -ini: Path to the igm.ini file (default: igm.ini)
  -m, --mem: Memory ceiling per job in GB (default: 4.0)
  -j, --joblog: Path to file where GNU parallel logs job run statistics (e.g. parallel_igm.log)
...
Example usage:
  ./igm.sh -n 4 -i 36
  ./igm.sh -n 4 -i 0 -j igm_jobs.log -r igm_logs/
  ./igm.sh -n 4 -i 0 -m 8.0 -f
...
# 1. Resolve igm.ini path
if [ -n "$FORECAST_IGM_INI" ] && [ -f "$FORECAST_IGM_INI" ]; then
    ini_file="$FORECAST_IGM_INI"
elif [ ! -f "$ini_file" ] && [ -f "igm/igm.ini" ]; then
    ini_file="igm/igm.ini"
fi

if [ ! -f "$ini_file" ]; then
    echo "Error: igm.ini configuration file not found at '$ini_file'."
    exit 1
fi

# 2. Extract output directory directly from igm.ini
...
echo "  Memory limit:$mem_ceiling GB"
...
# Function to run executable_igm with existence check
run_igm() {
    local snapshot=$1
    local plane=$2
    local out_dir=$3
    ...
    local expected_output="${out_dir}flux.igm.${snapshot}_${plane}.txt"
    ...
    ./executable_igm "${snapshot}" "${plane}" "${mem}" -ini "${ini_path}"
...
do
    ...
    then
        echo "${snaps[$i]} ${planes[$i]} ${output_dir} ${force_run} ${ini_file} ${mem_ceiling}"
    fi        
done | parallel --bar --jobs "$num_processors" "${parallel_args[@]}" --colsep ' ' run_igm {1} {2} {3} {4} {5} {6}
